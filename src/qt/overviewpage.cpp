...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSwapCashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSwapCashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSwapCashAmount = strAnonymizeSwapCashAmount.remove(strAnonymizeSwapCashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSwapCashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSwapCashAmount*COIN) nMaxToAnonymize = nAnonymizeSwapCashAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSwapCashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSwapCashAmount));
        strAnonymizeSwapCashAmount = strAnonymizeSwapCashAmount.remove(strAnonymizeSwapCashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSwapCashAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
