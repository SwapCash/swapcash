...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSwapCashAmount"))
        settings.setValue("nAnonymizeSwapCashAmount", 1000);
    nAnonymizeSwapCashAmount = settings.value("nAnonymizeSwapCashAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSwapCashAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeSwapCashAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSwapCashAmount:
            return QVariant(nAnonymizeSwapCashAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSwapCashAmount:
            nAnonymizeSwapCashAmount = value.toInt();
            settings.setValue("nAnonymizeSwapCashAmount", nAnonymizeSwapCashAmount);
            emit AnonymizeSwapCashAmountChanged(nAnonymizeSwapCashAmount);
            break;
        default:
            break;
        }
