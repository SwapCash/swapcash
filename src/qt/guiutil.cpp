...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("SwapCash"))
        return false;
...
{
    // Convert SwapCash:// to SwapCash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("SwapCash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "SwapCash:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SwapCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SwapCash.lnk
...
{
    return GetAutostartDir() / "SwapCash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a SwapCash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SwapCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the SwapCash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add SwapCash app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SwapCash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  SwapCash-qt [" + tr("command-line options") + "]                     " + "\n";
