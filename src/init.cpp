...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: SwapCash.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: swapcashd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeswapcashamount=<n> " + _("Keep N SwapCash anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SwapCash is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SwapCash is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SwapCash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "SwapCash server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SwapCash") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart SwapCash to complete") << "\n";
    ...
    nAnonymizeSwapCashAmount = GetArg("-anonymizeswapcashamount", 0);
    if(nAnonymizeSwapCashAmount > 999999) nAnonymizeSwapCashAmount = 999999;
    if(nAnonymizeSwapCashAmount < 2) nAnonymizeSwapCashAmount = 2;
    ...
    LogPrintf("Anonymize SwapCash Amount %d\n", nAnonymizeSwapCashAmount);
